...
class OptimizedPricePredictionModel:
    ...
    def prepare_advanced_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            self.logger.info(
                f"🔍 Advanced feature preparation with {len(df)} rows")

            df = df.copy().sort_values('timestamp')

            # FIXED: Better price column handling
            if 'price' in df.columns and df['price'].notna(
            ).sum() > len(df) * 0.3:
                price_col = 'price'
            ...
            if len(df) < 30:
                self.logger.error(
                    f"❌ Insufficient data after cleaning: {len(df)}")
                return pd.DataFrame()
            ...
            for period in [3, 5, 10, 15, 20]:
                df[f'sma_{period}'] = df[price_col].rolling(
                    period, min_periods=1).mean()
                df[f'price_vs_sma_{period}'] = (
                    df[price_col] - df[f'sma_{period}']) / (df[f'sma_{period}'] + 1e-8)
            ...
            df['momentum_3'] = df[price_col] / \
                (df[price_col].shift(3) + 1e-8) - 1
            df['momentum_5'] = df[price_col] / \
                (df[price_col].shift(5) + 1e-8) - 1
            df['momentum_10'] = df[price_col] / \
                (df[price_col].shift(10) + 1e-8) - 1

            # === VOLATILITY FEATURES ===
            for period in [5, 10, 15, 20]:
                vol_col = f'volatility_{period}'
                df[vol_col] = df[price_col].rolling(
                    period, min_periods=1).std()
                df[f'volatility_norm_{period}'] = df[vol_col] / \
                    (df[price_col] + 1e-8)
    ...
    (df['rsi_clean'] >= 40) & (
        df['rsi_clean'] <= 60)).astype(int)
            ...
            df['stoch_k'], df['stoch_d'] = self._calculate_stochastic_fixed(
                df[price_col])
    ...
    df['amount_in'], errors='coerce').fillna(1.0)
            ...
            df['volume_sma_5'] = df['volume_clean'].rolling(
                5, min_periods=1).mean()
            df['volume_sma_10'] = df['volume_clean'].rolling(
                10, min_periods=1).mean()
            df['volume_ratio'] = df['volume_clean'] / \
                (df['volume_sma_10'] + 1e-8)
            ...
            df['trend_strength'] = abs(
                df['sma_5'] - df['sma_20']) / (df['sma_20'] + 1e-8)
    ...
    (df['hour'] >= 22) | (
        df['hour'] <= 6)).astype(int)
            ...
            for col in feature_columns:
                if col in df.columns and not df[col].isna(
                ).all() and not np.isinf(df[col]).all():
                    available_features.append(col)

            self.logger.info(
                f"📋 Available features after filtering: {len(available_features)}")

            if len(available_features) < 8:
                self.logger.error(
                    f"❌ Too few valid features: {len(available_features)}")
                return pd.DataFrame()
    ...
    method='ffill').fillna(
        method='bfill')
            ...
            if len(final_clean) < 20:
                self.logger.error(
                    f"❌ Too few samples after cleaning: {len(final_clean)}")
                return pd.DataFrame()

            result_df = final_clean[available_features + ['target']].copy()

            self.logger.info(
                f"✅ Advanced features prepared: {result_df.shape}")
    ...
    df: pd.DataFrame,
     test_size: float = 0.2) -> Dict:
        ...
        try:
            self.logger.info(
                f"🤖 Training ensemble models with {len(df)} samples...")
    ...
    'success': False,
     'error': f'Too few clean samples: {len(X)}'}
            ...
            self.logger.info(
                f"📊 Training on {len(X_train)} samples, testing on {len(X_test)}")
    ...
    actual_direction == pred_direction) * 100
                    ...
                    self.logger.info(
                        f"✅ {model_name}: R²={test_r2:.3f}, Acc={accuracy:.1f}%, MAE={test_mae:.6f}")
    ...
    name for name,
     result in results.items() if result.get('success')]
            if successful_models:
                ...
                self.logger.info(
                    f"✅ Ensemble training complete! Successful models: {successful_models}")
            else:
    ...
    col for col in df_features.columns if col != 'target']
            ...
            # Scale features
            latest_features_scaled = self.feature_scaler.transform(
                latest_features)
            ...
            for model_name, model in self.models.items():
                try:
                    ...
                    self.logger.warning(
                        f"⚠️ {model_name} prediction failed: {e}")
