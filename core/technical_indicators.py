...
class TechnicalIndicators:
    ...
    def volume_profile(prices: List[float], volumes: List[float], bins: int = 20) -> Dict:
        ...
        if len(prices) != len(volumes) or len(prices) < bins:
            return {'vwap': prices[-1] if prices else 0.0, 'poc': prices[-1] if prices else 0.0}
        ...
        for i, price in enumerate(prices):
            bin_index = min(int((price - price_min) / (price_max - price_min) * (bins - 1)), bins - 2)
    ...
    def analyze_trend(self, prices: List[float]) -> Dict[str, any]:
        ...
        if len(prices) >= 5:
            ...
            if recent_change > 1.0:
                trend_score += 1
                reasons.append(f"Strong recent momentum (+{recent_change:.1f}%)")
