...
def main():
    ...
    tab1, tab2, tab3 = st.tabs(["📊 Statystyki", "📋 Ostatnie Transakcje", "📈 Analiza Czasowa"])
    ...
    with st.expander("🔍 System Info"):
        ...
        st.write(f"- Ostatnia aktualizacja: {df['timestamp'].max() if not df.empty else 'N/A'}")
        st.write(f"- Rozmiar danych: {os.path.getsize('data/memory.csv') / 1024:.1f} KB" if os.path.exists('data/memory.csv') else "- Plik danych niedostępny")
