...
def diagnose_ml_data():
    ...
    # KLUCZOWE KOLUMNY
    key_columns = ['price', 'volume', 'rsi', 'amount_in', 'amount_out', 'profitable']
    ...
    if 'profitable' in df.columns:
        ...
        print(f"True (profitable): {profitable_counts.get(True, 0)} ({profitable_counts.get(True, 0)/len(df)*100:.1f}%)")
        print(f"False (loss): {profitable_counts.get(False, 0)} ({profitable_counts.get(False, 0)/len(df)*100:.1f}%)")
    ...
    if len(numeric_cols) > 1:
        ...
        if 'profitable' in df.columns:
            ...
            correlations_with_target = df[numeric_cols].corrwith(df['profitable_numeric'])
    ...
    if 'price' in df.columns:
        ...
        if outliers > len(df) * 0.1:  # Więcej niż 10% outliers
            issues.append(f"Dużo outliers w price: {outliers} ({outliers/len(df)*100:.1f}%)")
    ...
    if 'profitable' in df.columns:
        balance_ratio = df['profitable'].value_counts().min() / df['profitable'].value_counts().max()
